# function [betahat, L, U] = CochranArmitage_CI_rx2(n, a, alpha, printresults)
CochranArmitage_CI_rx2 = function(n, a, alpha=0.05, printresults=T) {
    ...
    if (printresults) {
        print(sprintf('Trend estimate and Cochran-Armitage CI:  betahat = %6.4f (%g%% CI %6.4f to %6.4f)', 
            betahat, 100*(1 - alpha), L, U), quote=F)
    }
