MiettinenNurminen_asymptotic_score_CI_ratio_2x2 = function(n, alpha=0.05, printresults=T) {

    # function [L, U, estimate] = MiettinenNurminen_asymptotic_score_CI_ratio_2x2(n, alpha, printresults)
    ...
    if (n[1,1] == 0 && n[2,1] == 0) {
        ...
        L = uniroot(cll, c(phi0, phi1), n11=n11, n21=n21, n1p=n1p,
            n2p=n2p, pi1hat=pi1hat, pi2hat=pi2hat, alpha=alpha, tol=tol)$root
    } else if (estimate == 0) {
        L = 0
        # exitflag = 1
    } else {
        # [L, ~, exitflag] = fzero(@calculate_limit, [phi0, estimate], options)
        L = uniroot(cll, c(phi0, estimate), n11=n11, n21=n21, n1p=n1p,
            n2p=n2p, pi1hat=pi1hat, pi2hat=pi2hat, alpha=alpha, tol=tol)$root
    }
    ...
    if (is.na(estimate) || estimate==Inf) {
        ...
        U = uniroot(calculate_limit_upper, c(phi0, phi1), n11=n11, n21=n21, n1p=n1p,
            n2p=n2p, pi1hat=pi1hat, pi2hat=pi2hat, alpha=alpha, tol=tol)$root
    } else {
        # [U, ~, exitflag] = fzero(@calculate_limit, [estimate, phi1], options)
        U = uniroot(calculate_limit_upper, c(estimate, phi1), n11=n11, n21=n21, n1p=n1p,
            n2p=n2p, pi1hat=pi1hat, pi2hat=pi2hat, alpha=alpha, tol=tol)$root
    }
    # if exitflag ~= 1, display_warning(exitflag), }
...
# ================================
cll = function(phi0,n11,n21,n1p,n2p,pi1hat,pi2hat,alpha) {
