...
MiettinenNurminen_asymptotic_score_CI_ratio_2x2 <- function(n, alpha=0.05, printresults=TRUE) {
	...
	if (n[1, 1] == 0 && n[2, 1] == 0) {
		...
		L <- uniroot(
			...
			, c(phi0, phi1), n11=n11, n21=n21, n1p=n1p,
		...
		L <- uniroot(
			calculate_limit_lower.Miettinen_ratio, c(phi0, estimate), n11=n11, n21=n21, n1p=n1p,
			n2p=n2p, pi1hat=pi1hat, pi2hat=pi2hat, alpha=alpha, tol=tol
		)$root
	}
	...
	if (is.na(estimate) || estimate==Inf) {
		...
		U <- uniroot(
			calculate_limit_upper.Miettinen_ratio, c(phi0, phi1), n11=n11, n21=n21, n1p=n1p,
			n2p=n2p, pi1hat=pi1hat, pi2hat=pi2hat, alpha=alpha, tol=tol
		)$root
	} else {
		U <- uniroot(
			calculate_limit_upper.Miettinen_ratio, c(estimate, phi1), n11=n11, n21=n21, n1p=n1p,
			n2p=n2p, pi1hat=pi1hat, pi2hat=pi2hat, alpha=alpha, tol=tol
		)$root
	}
