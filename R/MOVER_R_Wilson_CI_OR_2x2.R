source("/Users/ole/Documents/Petter Laake/chap2_translation/Wilson_score_CI_1x2.R")

MOVER_R_Wilson_CI_OR_2x2 = function(n, alpha=0.05, printresults=T) {

    # function [L, U, estimate] = MOVER_R_Wilson_CI_OR_2x2(n, alpha, printresults)
    ...
    # printresults: display results (F = no, 1T= yes)
    ...
    if (n[2,2] == 0) {
        n = matrix(c(0, n[2,1], n[1,2], n[1,1]), nrow=2, byrow=T)
    }
    ...
    if (n[1,2] == 0) {
        # [~, U_tmp, ~] = MOVER_R_Wilson_CI_OR_2x2([0, n1p n[2,2], n[2,1]], alpha, 0)
        U_tmp = MOVER_R_Wilson_CI_OR_2x2(matrix(c(0,n1p,n[2,2],n[2,1]),nrow=2,byrow=T), alpha, printresults=F)$upper
        L = 1/U_tmp
    }
