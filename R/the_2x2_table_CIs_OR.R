...
my_sprintf_cat("Interval method                            %i%% CI      Log width\n", 100 * (1 - alpha))
  ...
  L <- res$lower
  U <- res$upper
  my_sprintf_cat("Woolf logit                           %6.3f to %6.3f  %7.3f\n", L, U, log(U) - log(L))

  res <- Gart_adjusted_logit_CI_2x2(n, alpha)
  L <- res$lower
  U <- res$upper
  my_sprintf_cat("Gart adjusted logit                   %6.3f to %6.3f  %7.3f\n", L, U, log(U) - log(L))

  res <- Independence_smoothed_logit_CI_2x2(n, alpha)
  L <- res$lower
  U <- res$upper
  my_sprintf_cat("Independence-smoothed logit           %6.3f to %6.3f  %7.3f\n", L, U, log(U) - log(L))
  ...
  L <- res$lower
  U <- res$upper
  my_sprintf_cat("Adjusted inverse sinh (0.45, 0.25)    %6.3f to %6.3f  %7.3f\n", L, U, log(U) - log(L))

  res <- Adjusted_inv_sinh_CI_OR_2x2(n, 0.6, 0.4, alpha)
  L <- res$lower
  U <- res$upper
  my_sprintf_cat("Adjusted inverse sinh (0.6, 0.4)      %6.3f to %6.3f  %7.3f\n", L, U, log(U) - log(L))
  ...
  L <- res$lower
  U <- res$upper
  my_sprintf_cat("Uncorrected asymptotic score          %6.3f to %6.3f  %7.3f\n", L, U, log(U) - log(L))

  res <- Cornfield_exact_conditional_CI_2x2(n, alpha)
  L <- res$lower
  U <- res$upper
  my_sprintf_cat("Cornfield exact conditional           %6.3f to %6.3f  %7.3f\n", L, U, log(U) - log(L))

  res <- BaptistaPike_exact_conditional_CI_2x2(n, alpha)
  L <- res$lower
  U <- res$upper
  my_sprintf_cat("Baptista-Pike exact conditional       %6.3f to %6.3f  %7.3f\n", L, U, log(U) - log(L))

  res <- Cornfield_midP_CI_2x2(n, alpha)
  L <- res$lower
  U <- res$upper
  my_sprintf_cat("Cornfield mid-P                       %6.3f to %6.3f  %7.3f\n", L, U, log(U) - log(L))

  res <- BaptistaPike_midP_CI_2x2(n, alpha)
  L <- res$lower
  U <- res$upper
  my_sprintf_cat("Baptista-Pike mid-P                   %6.3f to %6.3f  %7.3f\n", L, U, log(U) - log(L))
