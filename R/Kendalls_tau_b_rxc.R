# function [tau_b, L, U] = Kendalls_tau_b_rxc(n, alpha, printresults)

Kendalls_tau_b_rxc = function(n, alpha=0.05, printresults=T) {

	# Kendall's tau-b with confidence interval based on the Fieller standard deviation
	# Described in Chapter 7 "The rxc Table"
	# 
	# Dependencies: gamma_coefficient_rxc.m
	#
	# Input arguments
	# ---------------
	...
	if (printresults) {
	    print(sprintf('Kendalls tau-b w/Fieller CI: tau-b = %7.4f (%g%% CI %7.4f to %7.4f)', tau_b, 100*(1-alpha), L, U), quote=F)
	}
