 McNemar_exact_unconditional_test_paired_2x2 = function(n, gamma=0.0001, printresults=T) {
 	# The McNemar exact unconditional test
 	# Described in Chapter 8 "The Paired 2x2 Table"
 	#
 	# Dependencies: ClopperPearson_exact_CI_1x2_beta_version.m
 	#
 	# Input arguments
 	# ---------------
 	...
 	# maximization; however, it works well for most purposes. Try 'showplot=1'
 	...
 	if (gamma == 0) {
 	    ...
 	    tmp = ClopperPearson_exact_CI_1x2_beta_version(n[1,2] + n[2,1], N, gamma, F)
		L = tmp[[1]]; U = tmp[[2]]
 	...
 	P = max(Pvalues); index = which(P == Pvalues)[1]
