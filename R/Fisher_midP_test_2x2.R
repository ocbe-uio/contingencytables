Fisher_midP_test_2x2 = function(n, statistic='hypergeometric', printresults=T) {

    # function P = Fisher_midP_test_2x2(n, statistic, printresults)
    ...
    Tvalues = test_statistic_fisher_midp_test_2x2(x11values, n1p-x11values, np1-x11values, n2p-(np1-x11values), statistic)
    ...
    if (printresults) {
        if (statistic == 'hypergeometric') {
            print(sprintf('The Fisher mid-P test (Fisher-Irwin): P = %7.5f', P), quote=F)
        } else if (statistic == 'Pearson') {
            print(sprintf('The Fisher mid-P test (Pearson): P = %7.5f', P), quote=F)
        } else if (statistic == 'LR') {
            print(sprintf('The Fisher mid-P test (LR): P = %7.5f', P), quote=F)
        }
    }
