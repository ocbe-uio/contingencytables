Koopman_asymptotic_score_CI_2x2 = function(n, alpha=0.05, printresults=T) {

    # function [L, U, estimate] = Koopman_asymptotic_score_CI_2x2(n, alpha, printresults)
    # The Koopman asymptotic score confidence interval for the ratio of probabilities
    # Described in Chapter 4 "The 2x2 Table"
    # This versions uses the score test statistic of the Miettinen-Nurminen
    ...
    if (n[1,1] == 0 && n[2,1] == 0) {
        ...
        # [L, ~, exitflag] = fzero(@calculate_limit, [phi0, phi1], options)
        L = uniroot(calculate_limit_lower, c(phi0, phi1), n11=n11, n21=n21, 
            n1p=n1p, n2p=n2p, pi1hat=pi1hat, pi2hat=pi2hat, alpha=alpha, tol=tol)$root
    } else if (estimate == 0) {
        L = 0 
        # exitflag = 1
    } else {
        # [L, ~, exitflag] = fzero(@calculate_limit, [phi0, estimate], options)
        L = uniroot(calculate_limit_lower, c(phi0, estimate), n11=n11, n21=n21, 
            n1p=n1p, n2p=n2p, pi1hat=pi1hat, pi2hat=pi2hat, alpha=alpha, tol=tol)$root
    }
    ...
    if (is.na(estimate) || estimate==Inf) {
        ...
        # [U, ~, exitflag] = fzero(@calculate_limit, [phi0, phi1], options)
        U = uniroot(calculate_limit_upper, c(phi0, phi1), n11=n11, n21=n21, 
            n1p=n1p, n2p=n2p, pi1hat=pi1hat, pi2hat=pi2hat, alpha=alpha, tol=tol)$root
    } else {
        # [U, ~, exitflag] = fzero(@calculate_limit, [estimate, phi1], options)
        U = uniroot(calculate_limit_upper, c(estimate, phi1), n11=n11, n21=n21, 
            n1p=n1p, n2p=n2p, pi1hat=pi1hat, pi2hat=pi2hat, alpha=alpha, tol=tol)$root
    }
    # if exitflag ~= 1, display_warning(exitflag), }
...
calculate_limit_lower = function(phi0, n11, n21, n1p, n2p, pi1hat, pi2hat, alpha) {
    ...
    # [p1hat, p2hat] = ML_estimates(n11, n21, n1p, n2p, phi0)
    ml.res = ML_estimates(n11, n21, n1p, n2p, phi0)
    T0 = score_test_statistic(pi1hat, pi2hat, ml.res$p1hat, ml.res$p2hat, n1p, n2p, phi0)
    if (is.na(T0)) {
        T0 = 0
    }
...
# ==============================================================
# function [p1hat, p2hat] = ML_estimates(n11, n21, n1p, n2p, phi0)
