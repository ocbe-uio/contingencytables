# function [tau_b, L, U] = Kendalls_tau_b_rxc_bca(n, nboot, alpha, printresults)

Kendalls_tau_b_rxc_bca = function(n, nboot=10000, alpha=0.05, printresults=T) {

	# Kendall's tau-b with the bias-corrected and accelerated boostrap confidence interval
	# Described in Chapter 7 "The rxc Table"
	#
	# Dependencies: Kendalls_tau_b_rxc.m
	#
	# Input arguments
	# ---------------
	...
	if (printresults) {
	    print(sprintf('Kendalls tau-b w/BCa bootstrap CI: tau-b = %7.4f (%g%% CI %7.4f to %7.4f)', tau_b, 100*(1-alpha), L, U), quote=F)
	}
...
# ===================================================
# function tau_b = put_data_back_into_table_format(Y1, Y2)
...
f.Ktbrb = function(dat, d, .alpha, .r, .c) {
	...
	res = Kendalls_tau_b_rxc(n, .alpha, printresults=F)
	return(res$tau_b)
}
