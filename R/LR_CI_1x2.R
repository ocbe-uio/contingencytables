LR_CI_1x2 = function(X, n, alpha=0.05, printresults=T) {
    ...
    # Define global variables that are needed in the LR test statistic function below
    ...
    if (estimate == 0) {
        ...
        L = uniroot(LR_test_statistic, interval=c(tol,X/n), X=X, n=n, z=z, tol=tol)$root
    }
    ...
    if (estimate == 1) {
        ...
        # [U, ~, ~] = fzero(@LR_test_statistic, [max([tol, X/n]), 1 - tol], options);
        U = uniroot(LR_test_statistic, interval=c(max(tol,X/n),1-tol), X=X, n=n, z=z, tol=tol)$root
    }

    if (printresults) {
        print(sprintf('The likelihood ratio CI: estimate = %6.4f (%g%% CI %6.4f to %6.4f)', 
