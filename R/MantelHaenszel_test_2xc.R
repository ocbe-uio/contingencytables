# function [P, T, df] = MantelHaenszel_test_2xc(n, b, printresults) 

MantelHaenszel_test_2xc = function(n, b, printresults=T) {  
...
# The two-sided P-value (reference distribution: chi-squared with one degree of freedom)
df = 1
P = 1 - pchisq(T0, df)

if (printresults) {
    print(sprintf('Mantel-Haenszel test of association: P = %6.4f, T = %5.3f (df=%g)', P, T0, df), quote=F)
}
