path = "/Users/ole/Desktop/Petter Laake/chap7_translation"

source(paste(path, "Bonferroni_type_CIs_rxc.R", sep="/"))
source(paste(path, "Cumulative_models_for_rxc.R", sep="/"))
source(paste(path, "Exact_cond_midP_tests_rxc.R", sep="/"))
source(paste(path, "FisherFreemanHalton_asymptotic_test_rxc.R", sep="/"))
source(paste(path, "gamma_coefficient_rxc_bca.R", sep="/"))
source(paste(path, "gamma_coefficient_rxc.R", sep="/"))
source(paste(path, "JonckheereTerpstra_test_rxc.R", sep="/"))
source(paste(path, "Kendalls_tau_b_rxc_bca.R", sep="/"))
source(paste(path, "Kendalls_tau_b_rxc.R", sep="/"))
source(paste(path, "KruskalWallis_asymptotic_test_rxc.R", sep="/"))
source(paste(path, "linear_by_linear_test_rxc.R", sep="/"))
source(paste(path, "Pearson_correlation_coefficient_rxc_bca.R", sep="/"))
source(paste(path, "Pearson_correlation_coefficient_rxc.R", sep="/"))
source(paste(path, "Pearson_LR_tests_rxc.R", sep="/"))
source(paste(path, "Pearson_residuals_rxc.R", sep="/"))
source(paste(path, "Scheffe_type_CIs_rxc.R", sep="/"))
source(paste(path, "Spearman_correlation_coefficient_rxc_bca.R", sep="/"))
source(paste(path, "Spearman_correlation_coefficient_rxc.R", sep="/"))

the_rxc_table = function(n, alpha=0.05) {
 	
 	if (missing(n)) {
 	# Unordered tables
 	#    n = [40 25; 54 7; 63 10]; # Treatment for ear infection (van Balen et al., 2003)
 	#    n = [62 21; 97 48; 10 12; 30 7; 132 78; 34 17]; # Psychiatric diagnoses vs PA (Mangerud et al., 2004)
 	#    n = [3 55 23; 8 102 36; 6 14 1; 5, 21, 12; 19 130 64; 7 26 18]; # Psychiatric diag. vs BMI (Mangerud et al., 2004)
 	
 	# Singly ordered tables
 	#    n = [3 55 23; 8 102 36; 6 14 1; 5, 21, 12; 19 130 64; 7 26 18]; # Psychiatric diag. vs BMI (Mangerud et al., 2004)
 	#    n = [22 4 12; 24 9 10; 51 7 6]; # Low birth weight vs psychiatric morbitidy (Lund et al., 2012)
 	...
 	results = Pearson_LR_tests_rxc(n,printresults=F)
 	...
 	tmp = FisherFreemanHalton_asymptotic_test_rxc(n, printresults=F)
 	...
 	if(r <= 3 && c <= 2) {
 	    results_exact_midP = Exact_cond_midP_tests_rxc(n, printresults=F)
 	...
 	tmp = Pearson_residuals_rxc(n, printresults=F)
 	residuals = tmp[[1]]
 	std_residuals = tmp[[2]]
 	...
 	if(c == 2) {
 	    Scheffe_type_CIs_rxc(n, alpha, printresults=T)
        .print('\n')
 	    Bonferroni_type_CIs_rxc(n, alpha, printresults=T)
        .print('\n')
 	}
    ...
    .print('Method                                     Statistic      P-value\n')
 	...
 	tmp = KruskalWallis_asymptotic_test_rxc(n, printresults=F)
 	P = tmp[[1]]
 	T0 = tmp[[2]]
 	df = tmp[[3]]
 	...
 	# (The first row needs to be the reference category. Swap rows to get it right.)
 	n0 = rbind(c(22, 4, 12), c(24, 9, 10), c(51, 7, 6))
 	if (dim(n) == dim(n0) && n == n0) {
 		n = rbind(c(51,7,6),c(22,4,12),c(24,9,10))
 	}
 	n0 = rbind(c(3, 55, 23), c(8, 102, 36), c(6, 14, 1), c(5, 21, 12), c(19, 130, 64), c(7, 26, 18))
 	if (dim(n) == dim(n0) && n == n0) {
 	    n = rbind(c(19,130,64), c(3,55,23), c(8,102,36), c(6,14,1), c(5,21,12), c(7,26,18))  # Hyperkinetic disorders as ref. category
 	    # n = [6 14 1; 3 55 23; 8 102 36; 5, 21, 12; 19 130 64; 7 26 18];  # Eating disorders as ref. category
 	}
 	
 	results = Cumulative_models_for_rxc(n, 'logit', alpha, printresults=F)
 	...
 	tmp = linear_by_linear_test_rxc(n, 1:r, 1:c, printresults=F)
 	P = tmp[[1]]
 	Z = tmp[[2]]
 	.print('  Linear-by-linear                         %6.3f    %9.6f\n', Z, P)
 	tmp = JonckheereTerpstra_test_rxc(n, printresults=F)
 	P = tmp[[1]]
 	Z = tmp[[2]]
 	...
 	tmp = Pearson_correlation_coefficient_rxc(n, 1:r, 1:c, alpha, printresults=F)
 	rP = tmp[[1]]; L = tmp[[2]]; U = tmp[[3]]
 	.print('Pearson correlation coefficient           %6.3f (%g%% CI %6.3f to %6.3f)\n', rP, 100*(1-alpha), L, U)
 	tmp = Pearson_correlation_coefficient_rxc_bca(n, nboot, 1:r, 1:c, alpha, printresults=F)
 	rP = tmp[[1]]; L = tmp[[2]]; U = tmp[[3]]
 	.print('Pearson correlation w/BCa bootstrap CI    %6.3f (%g%% CI %6.3f to %6.3f), nboot = %g\n', rP, 100*(1-alpha), L, U, nboot)
 	
 	cat("\n")
 	# The colorectal cancer example with mid-interval scores
 	n0 = rbind(c(2, 4, 29, 19), c(7, 6, 116, 51), c(19, 27, 201, 76), c(18, 22, 133, 54))
 	if (dim(n) == dim(n0) && n == n0) {
 	    a = c(0.5, 5, 17, 39)
 	    tmp = Pearson_correlation_coefficient_rxc(n, a, 1:c, alpha, printresults=F)
 	    rP = tmp[[1]]; L = tmp[[2]]; U = tmp[[3]]
     	.print('\nPearson w/mid-interval scores             %6.3f (%g%% CI %6.3f to %6.3f)\n', rP, 100*(1-alpha), L, U)
 	    tmp = Pearson_correlation_coefficient_rxc_bca(n, nboot, a, 1:c, alpha, printresults=F)
 	    rP = tmp[[1]]; L = tmp[[2]]; U = tmp[[3]]
     	.print('Pearson w/mid-interval scores & BCa CI    %6.3f (%g%% CI %6.3f to %6.3f), nboot = %g\n', rP, 100*(1-alpha), L, U, nboot)
 	}
 	
 	cat("\n")
 	tmp = Spearman_correlation_coefficient_rxc(n, alpha, printresults=F)
 	rho = tmp[[1]]; L = tmp[[2]]; U = tmp[[3]]; L_BW = tmp[[4]]; U_BW = tmp[[5]]
 	...
 	tmp = Spearman_correlation_coefficient_rxc_bca(n, nboot, alpha, printresults=F)
 	rho = tmp[[1]]; L = tmp[[2]]; U = tmp[[3]]
 	.print('Spearman correlation w/BCa bootstrap CI   %6.3f (%g%% CI %6.3f to %6.3f), nboot = %g\n', rho, 100*(1-alpha), L, U, nboot)
 	
 	cat("\n")
 	gamma = gamma_coefficient_rxc(n, printresults=F)$gamma
 	.print('\nThe gamma coefficient                     %6.3f\n', gamma)
 	tmp = gamma_coefficient_rxc_bca(n, nboot, alpha, printresults=F)
 	gamma = tmp[[1]]; L = tmp[[2]]; U = tmp[[3]]
 	.print('The gamma coefficient w/BCa bootstrap CI  %6.3f (%g%% CI %6.3f to %6.3f), nboot = %g\n', gamma, 100*(1-alpha), L, U, nboot)
 	
 	cat("\n")
 	tmp = Kendalls_tau_b_rxc(n, alpha, printresults=F)
 	tau_b = tmp[[1]]; L = tmp[[2]]; U = tmp[[3]]
 	.print('\nKendalls tau-b w/Fieller CI              %6.3f (%g%% CI %6.3f to %6.3f)\n', tau_b, 100*(1-alpha), L, U)
 	tmp = Kendalls_tau_b_rxc_bca(n, nboot, alpha, printresults=F)
 	tau_b = tmp[[1]]; L = tmp[[2]]; U = tmp[[3]]
