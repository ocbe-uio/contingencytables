...
the_paired_cxc_table_nominal <- function(n, alpha = 0.05) {
  ...
  my_sprintf("-----------------------------------\n")
  for (i in 1:c) {
    my_sprintf("Marginal proportion %g: %5.3f  %5.3f\n", i, sum(n[i, ]) / N, sum(n[, i]) / N)
  }
  my_sprintf("-----------------------------------\n")
  ...
  T0 <- tmp[[2]]
  df <- tmp[[3]]
  my_sprintf("McNemar-Bowker test for symmetry           %6.3f (df=%g)  %9.6f\n", T0, df, P)
  my_sprintf("-------------------------------------------------------------------\n")
