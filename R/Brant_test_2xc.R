# function [P, T, df] = Brant_test_2xc(n, printresults)
	
Brant_test_2xc = function(n, printresults=T) {
	...
	x = rep(0,N); y = rep(0,N); z = matrix(0, N, c0-1); id = 1
	for (i in 1:r0) {
	    for (j in 1:c0) {
	        x[id:(id+n[i,j]-1)] = i-1
	        y[id:(id+n[i,j]-1)] = j
	...
	if (printresults) {
	    print(sprintf('Brant test: T = %6.3f, df = %g, P = %7.5f', T0, df, P0), quote=F)
	}
