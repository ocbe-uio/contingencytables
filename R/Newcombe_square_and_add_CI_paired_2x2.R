 Newcombe_square_and_add_CI_paired_2x2 = function(n, alpha=0.05, printresults=T) {
 	# The Newcombe square-and-add confidence interval for the difference
 	# between paired probabilities
 	...
 	tmp = Wilson_score_CI_1x2(nip[1], N, alpha, printresults=F)
	l1 = tmp[[1]]
	u1 = tmp[[2]]
 	tmp = Wilson_score_CI_1x2(npi[1], N, alpha, printresults=F)
	l2 = tmp[[1]]
	u2 = tmp[[2]]
