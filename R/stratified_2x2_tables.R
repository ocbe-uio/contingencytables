path = "/Users/ole/Desktop/Petter Laake/chap10_translation"
source(paste(path, "MantelHaenszel_estimate_stratified_2x2.R", sep="/"))
source(paste(path, "InverseVariance_estimate_stratified_2x2.R", sep="/"))
source(paste(path, "Peto_OR_estimate_stratified_2x2.R", sep="/"))
source(paste(path, "Cochran_Q_test_stratified_2x2.R", sep="/"))
source(paste(path, "Pearson_LR_homogeneity_test_stratified_2x2.R", sep="/"))
source(paste(path, "BreslowDay_homogeneity_test_stratified_2x2.R", sep="/"))
source(paste(path, "Peto_homogeneity_test_stratified_2x2.R", sep="/"))
source(paste(path, "Pearson_LR_test_common_effect_stratified_2x2.R", sep="/"))
source(paste(path, "Wald_test_and_CI_common_diff_stratified_2x2.R", sep="/"))
source(paste(path, "ML_estimates_and_CIs_stratified_2x2.R", sep="/"))
source(paste(path, "Pearson_LR_test_common_effect_stratified_2x2.R", sep="/"))
source(paste(path, "Wald_test_and_CI_common_ratio_stratified_2x2.R", sep="/"))
source(paste(path, "ML_estimates_and_CIs_stratified_2x2.R", sep="/"))
source(paste(path, "CochranMantelHaenszel_test_stratified_2x2.R", sep="/"))
source(paste(path, "RBG_test_and_CI_stratified_2x2.R", sep="/"))
source(paste(path, "Woolf_test_and_CI_stratified_2x2.R", sep="/"))
...
stratified_2x2_tables = function(n, alpha=0.05) {
	...
	P=tmp[[1]]; Z=tmp[[2]]; L_MH=tmp[[3]]; U_MH=tmp[[4]]; deltahat_MH=tmp[[5]]
 	.print('Wald (MH)            %6.4f   (Z = %5.3f)\n',P, Z)
 	tmp = Wald_test_and_CI_common_diff_stratified_2x2(n, 'IV', alpha, F)
	P=tmp[[1]]; Z=tmp[[2]]; L_IV=tmp[[3]]; U_IV=tmp[[4]]; deltahat_IV=tmp[[5]]
	...
	P=tmp[[1]]; Z=tmp[[2]]; L_RBG=tmp[[3]]; U_RBG=tmp[[4]]; phihat_RBG=tmp[[5]]
 	.print('RBG                      %6.4f   (Z = %5.3f)\n', P, Z)
 	tmp = Woolf_test_and_CI_stratified_2x2(n, alpha, F)
	P=tmp[[1]]; Z=tmp[[2]]; L_Woolf=tmp[[3]]; U_Woolf=tmp[[4]]; phihat_Woolf=tmp[[5]]
