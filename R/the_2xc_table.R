path = getwd()
source(paste(path, "Brant_test_2xc.R", sep="/"))
source(paste(path, "Cumulative_models_for_2xc.R", sep="/"))
source(paste(path, "Exact_cond_midP_linear_rank_tests_2xc.R", sep="/"))
source(paste(path, "Exact_cond_midP_unspecific_ordering_rx2.R", sep="/"))
source(paste(path, "MantelHaenszel_test_2xc.R", sep="/"))
source(paste(path, "Pearson_LR_tests_cum_OR_2xc.R", sep="/"))
source(paste(path, "Pearson_LR_tests_rxc.R", sep="/"))
source(paste(path, "Pearson_LR_tests_unspecific_ordering_rx2.R", sep="/"))
source(paste(path, "Score_test_for_effect_in_the_probit_model_2xc.R", sep="/"))

the_2xc_table = function(n, alpha=0.05, direction="increasing") {
	
	if (missing(n)) {
	    # The Adolescent Placement Study (Fontanella et al., 2008)
		#   n = rbind(c(8, 28, 72, 126), c(46, 73, 69, 86)); direction = "increasing"	
