Goodman_Wilson_score_CIs_1xc = function(n, alpha=0.05, printresults=T) {

    # function [L, U, pihat] = Goodman_Wilson_score_CIs_1xc(n, alpha, printresults)
    ...
    for (i in 1:c0) {
        L[i] = (Bonferroni + 2*N*pihat[i] - sqrt(Bonferroni^2 + 4*N*Bonferroni*pihat[i]*(1 - pihat[i])))/(2*Bonferroni + 2*N)
        U[i] = (Bonferroni + 2*N*pihat[i] + sqrt(Bonferroni^2 + 4*N*Bonferroni*pihat[i]*(1 - pihat[i])))/(2*Bonferroni + 2*N)
    }

    if (printresults) {
        print(sprintf('The Goodman Wilson score simultaneous intervals'), quote=F)
        for (i in 1:c0) {
