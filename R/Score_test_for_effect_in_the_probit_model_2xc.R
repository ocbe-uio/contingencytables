...
Score_test_for_effect_in_the_probit_model_2xc = function(n, alphahat0, printresults=T) {
	...
	# Input arguments
	# ---------------
	# n: the observed counts (a 2xc matrix)
	# alphahat0: a column vector with c-1 estimated coefficients (alpha_j)
	...
	# because Matlab does not calculate an intercept-only probit model.
	# alphahat0 can be calculated in, for instance, Stata
	
	if (missing(alphahat0)) {
	    # The Adolescent Placement Study (Fontanella et al., 2008)
		# n = [8, 28, 72, 126; 46, 73, 69, 86]; alphahat0 = [-1.246452, -0.5097363, 0.2087471]';
	...
	if (printresults) {
	    print(sprintf('Score test for effect: P = %6.4f, T = %5.3f (df=%g)', P, T0, df), quote=T)
	}
...
first_derivative_wrt_alpha = function(n, c, alpha, beta) {
	...
	for (k in 1:(c-1)) {
	    for (j in 1:c) {
	        ...
	        PHI_j = pnorm(alpha[j+1] - beta)
	        PHI_jminus1 = pnorm(alpha[j] - beta)
	        dldalpha[k] = dldalpha[k] + n[1,j]*(jk*phi_j - jk1*phi_jminus1)/(PHI_j - PHI_jminus1)   
	
	        phi_j = dnorm(alpha[j+1])
	        phi_jminus1 = dnorm(alpha[j])
	        PHI_j = pnorm(alpha[j+1])
	        PHI_jminus1 = pnorm(alpha[j])
	        dldalpha[k] = dldalpha[k] + n[2,j]*(jk*phi_j - jk1*phi_jminus1)/(PHI_j - PHI_jminus1)  
	    }
	}
	return(dldalpha)
}
...
second_derivative_wrt_beta = function(n, c, alpha, beta) {
	...
	for (j in 1:c) {
	    ...
	    d2ldb2 = d2ldb2 + n[1,j]*(PHI_j - PHI_jminus1)*(term1 - term2)/((PHI_j - PHI_jminus1)^2)
	    d2ldb2 = d2ldb2 - n[1,j]*((phi_jminus1 - phi_j)^2)/((PHI_j - PHI_jminus1)^2)
	}
	return(d2ldb2)
}
...
second_derivative_wrt_beta_and_alpha = function(n, c, alpha, beta) {
	...
	for (l in 1:(c-1)) {
	    for (j in 1:c) {
	        ...
	        d2ldbdalpha[l] = d2ldbdalpha[l] + n[1,j]*(PHI_j - PHI_jminus1)*(jl*term1 - jl1*term2)/((PHI_j - PHI_jminus1)^2)
	        d2ldbdalpha[l] = d2ldbdalpha[l] - n[1,j]*(phi_jminus1 - phi_j)*(jl*phi_j - jl1*phi_jminus1)/((PHI_j - PHI_jminus1)^2)
	    }
	}
	return(d2ldbdalpha)
}
	...
	for (k in 1:(c-1)) {
	    for (l in 1:(c-1)) {
	        for (j in 1:c) {
	            ...
	            d2ldalpha2[k,l] = d2ldalpha2[k,l] + n[1,j]*((PHI_j-PHI_jminus1)*(jk1*jl1*term1-jk*jl*term2))/((PHI_j-PHI_jminus1)^2)
	            d2ldalpha2[k,l] = d2ldalpha2[k,l] - n[1,j]*((jk*phi_j-jk1*phi_jminus1)*(jl*phi_j-jl1*phi_jminus1))/((PHI_j-PHI_jminus1)^2)             
	            phi_j = dnorm(alpha[j+1])
	            phi_jminus1 = dnorm(alpha[j])
	            PHI_j = pnorm(alpha[j+1])
	            PHI_jminus1 = pnorm(alpha[j])
	            ...
	            d2ldalpha2[k,l] = d2ldalpha2[k,l] + n[2,j]*((PHI_j-PHI_jminus1)*(jk1*jl1*term1-jk*jl*term2))/((PHI_j-PHI_jminus1)^2)
	            d2ldalpha2[k,l] = d2ldalpha2[k,l] - n[2,j]*((jk*phi_j-jk1*phi_jminus1)*(jl*phi_j-jl1*phi_jminus1))/((PHI_j-PHI_jminus1)^2)
	        }
	    }
	}
	return(d2ldalpha2) 
