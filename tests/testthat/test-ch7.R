...
test_that("Chapter 7 functions basically work", {
  ...
  expect_output(
    object = Bonferroni_type_CIs_rxc(table_7.3),
  ...
  expect_output(
    object = FisherFreemanHalton_asymptotic_test_rxc(table_7.3),
  ...
  expect_output(
    object = JonckheereTerpstra_test_rxc(table_7.7),
  ...
  expect_output(
    object = KruskalWallis_asymptotic_test_rxc(table_7.6),
    regexp = "Asymptotic Kruskal-Wallis test: T =  9.162, df = 2, P = 0.010"
  )
  expect_output(
    object = linear_by_linear_test_rxc(table_7.7),
  ...
  expect_equal(
    object = dim(Pearson_residuals_rxc(table_7.3, printresults = FALSE)$residuals),
    expected = c(3, 2)
  )
  expect_output(
    object = Scheffe_type_CIs_rxc(table_7.3),
