...
test_that("Pearson_chi_squared_test_CC_2x2 equals chisq.test", {
  expect_equal(
    Pearson_chi_squared_test_CC_2x2(n1)$statistics$p.value,
    suppressWarnings(chisq.test(n1, correct = TRUE)$p.value)
  )
  expect_equal(
    Pearson_chi_squared_test_CC_2x2(n2)$statistics$p.value,
    suppressWarnings(chisq.test(n2, correct = TRUE)$p.value)
  )
  expect_equal(
    Pearson_chi_squared_test_CC_2x2(n3)$statistics$p.value,
    suppressWarnings(chisq.test(n3, correct = TRUE)$p.value)
  )
  expect_equal(
    Pearson_chi_squared_test_CC_2x2(n4)$statistics$p.value,
    suppressWarnings(chisq.test(n4, correct = TRUE)$p.value)
  )
  expect_equal(
    Pearson_chi_squared_test_CC_2x2(x)$statistics$p.value,
